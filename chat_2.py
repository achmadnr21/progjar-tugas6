...
class Chat:
	def __init__(self):
		...
		self.sessions={}
		self.users = {}
		self.users['rudy']={ 'nama': 'Achmad NashruddinR', 'negara': 'Indonesia', 'password': 'bojonegoro', 'incoming' : {}, 'outgoing': {}}
		self.users['sasha']={ 'nama': 'Tamara Sasha', 'negara': 'Russia', 'password': 'moskva', 'incoming': {}, 'outgoing': {}}
		self.users['albert']={ 'nama': 'Albert Einstein', 'negara': 'Jerman', 'password': 'berlin','incoming': {}, 'outgoing':{}}
	...
	def proses(self, data, socket=[]):
		...
		try:
			...
			if (command=='auth'):
				...
				logging.warning("SEND: [{}] send message from {} to {}" . format(sessionid, usernamefrom,usernameto))
	...
	def group_chat(self, username, groupname, state, socket):
		...
		if groupname not in groups and state != 'comeback':
			...
			return {'status': 'PENDING', 'message': f'Group::{groupname} not found!'}
		elif state == 'comeback':
		...
		while True:
			try:
				...
				if chat != 'exit':
					...
					self.broadcast(groupname, f"{username} left!")
					break
			except:
				self.exitGroup(groupname, client)
				break
		...
		return {'status': 'OK', 'message': f'Leaving {groupname}'}
